...
const GeneralContextProvider = ({children}) => {
  ...
  const fetchCartCount = async() =>{
    ...
    if(userId){
      await axios.get(`${process.env.REACT_APP_BASE_API_URL}/fetch-cart`).then(
        (response)=>{
  ...
  const login = async () =>{
    try{
      const loginInputs = {email, password}
        await axios.post(`${process.env.REACT_APP_BASE_API_URL}/login`, loginInputs)
        .then( async (res)=>{
  ...
  const register = async () =>{
    try{
        await axios.post(`${process.env.REACT_APP_BASE_API_URL}/register`, inputs)
        .then( async (res)=>{
            localStorage.setItem('userId', res.data._id);
            localStorage.setItem('userType', res.data.usertype);
            localStorage.setItem('username', res.data.username);
